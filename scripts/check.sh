...
bin="$repo_dir/bin/codexhud"
export CODEXHUD_PRICE_FILE="$repo_dir/config/prices.json"
...
if command -v shellcheck >/dev/null 2>&1; then
  shellcheck "$bin" "$repo_dir/install.sh" "$repo_dir/tmux/codexhud.tmux"
elif [ "${CODEXHUD_REQUIRE_SHELLCHECK:-0}" = "1" ]; then
...
cache_key="$("$bin" cache-key /tmp/codexhud-target)"
case "$cache_key" in
  ????????????????) ;;
  *)
...
case "$init_bash" in
  *"codexhud cache-key \"\$PWD\""*) ;;
  *)
    printf 'expected init hook to use codexhud cache-key\n' >&2
    exit 1
    ;;
esac
case "$init_bash" in
  *'refresh.log'*) ;;
  *)
    printf 'expected init hook to include CODEXHUD_DEBUG refresh logging\n' >&2
    exit 1
    ;;
esac
...
{"type":"session_meta","payload":{"id":"target-session","timestamp":"2026-04-29T01:00:00.000Z","cwd":"/tmp/codexhud-target","originator":"codex-tui","cli_version":"0.125.0","source":"cli","model_provider":"openai","model":"gpt-5.4"}}
...
{"type":"session_meta","payload":{"id":"other-session","timestamp":"2026-04-29T01:01:00.000Z","cwd":"/tmp/codexhud-other","originator":"codex-tui","cli_version":"0.125.0","source":"cli","model_provider":"openai"}}
...
for style in balanced minimal ledger risk executive focus ascii; do
  CODEX_HOME="$codex_home" CODEXHUD_STYLE="$style" "$bin" tmux /tmp/codexhud-target >/dev/null
done

json_for_target="$(CODEX_HOME="$codex_home" "$bin" json /tmp/codexhud-target)"
...
tmux_for_target="$(CODEX_HOME="$codex_home" "$bin" tmux /tmp/codexhud-target)"
...
unknown_model="$(CODEX_HOME="$codex_home" CODEXHUD_MODEL="unknown-test-model" "$bin" json /tmp/codexhud-target)"
printf '%s\n' "$unknown_model" | jq -e '
...
stale_model="$(CODEX_HOME="$codex_home" CODEXHUD_PRICE_FILE="$stale_prices" "$bin" json /tmp/codexhud-target)"
printf '%s\n' "$stale_model" | jq -e '
...
touch "$cache_file"
CODEX_HOME="$codex_home" CODEXHUD_CACHE_FILE="$cache_file" "$bin" cache-refresh /tmp/codexhud-target
test -s "$cache_file"
CODEX_HOME="$codex_home" CODEXHUD_CACHE_FILE="$cache_file" "$bin" cache-refresh /tmp/codexhud-target
test -s "$cache_file"
...
cache_from_home="$(
  CODEXHUD_HOME="$custom_home" "$bin" init bash \
    | awk '/CODEXHUD_CACHE_DIR:-/ { print; exit }'
)"
expected_cache_expr="\${CODEXHUD_HOME:-\$HOME/.codexhud}/cache"
case "$cache_from_home" in
  *"$expected_cache_expr"*) ;;
  *)
    printf 'expected init hook to honor CODEXHUD_HOME, got: %s\n' "$cache_from_home" >&2
    exit 1
    ;;
esac
